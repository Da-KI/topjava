...
public class JdbcMealRepository implements MealRepository {
    ...
    public boolean delete(int id, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                .addValue("id", id)
                .addValue("userId", userId);
        return namedParameterJdbcTemplate.update("DELETE FROM meals WHERE id=:id AND user_id=:userId", map) != 0;
    }
    ...
    public Meal get(int id, int userId) {
        MapSqlParameterSource map = new MapSqlParameterSource()
                .addValue("id", id)
                .addValue("userId", userId);
        List<Meal> meals = namedParameterJdbcTemplate.query("SELECT * FROM meals WHERE id=:id AND user_id=:userId", map, ROW_MAPPER);
